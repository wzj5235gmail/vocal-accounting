...
type TimeRange = 'all' | 'thisWeek' | 'thisMonth' | 'thisYear' | 'lastWeek' | 'lastMonth' | 'lastYear' | 'custom';
type GroupBy = 'category' | 'day' | 'month' | 'year';

export default function StatisticsSection({ expenses }: StatisticsSectionProps) {
  ...
  useEffect(() => {
    ...
    const dataToProcess = filteredExpenses.length > maxItemsToProcess
    ...
    setTimeout(() => {
      ...
      // 根据分组方式计算统计数据
      let grouped: Record<string, { amount: number, count: number, expenses: Expense[] }> = {};

      switch (groupBy) {
        case 'category':
          dataToProcess.forEach(e => {
            if (!grouped[e.category]) {
              grouped[e.category] = { amount: 0, count: 0, expenses: [] };
            }
        ...
        case 'day':
          dataToProcess.forEach(e => {
            const day = e.date;
            if (!grouped[day]) {
              grouped[day] = { amount: 0, count: 0, expenses: [] };
            }
            grouped[day].amount += e.amount;
            grouped[day].count += 1;
            grouped[day].expenses.push(e);
          });
          break;
        ...
        case 'month':
          dataToProcess.forEach(e => {
            ...
            if (!grouped[month]) {
              grouped[month] = { amount: 0, count: 0, expenses: [] };
            }
            grouped[month].amount += e.amount;
        ...
        case 'year':
          dataToProcess.forEach(e => {
            ...
            if (!grouped[year]) {
              grouped[year] = { amount: 0, count: 0, expenses: [] };
            }
            grouped[year].amount += e.amount;
            grouped[year].count += 1;
            grouped[year].expenses.push(e);
          });
          break;
      }
  ...
  useEffect(() => {
    ...
    async function calculateConvertedTotal() {
      ...
      try {
        // 使用批处理来减少API调用
        // 按币种分组并一次性转换
        const total = await convertExpensesToCurrency(filteredExpenses, selectedCurrency);
        ...
        if (isMounted) {
          ...
          if (stats.length > 0) {
            const updatedStats = await Promise.all(stats.map(async (item) => {
              ...
              if (groupBy === 'category') {
                // 找出该分类的所有支出
                const categoryExpenses = filteredExpenses.filter(e => e.category === item.label);
                // 转换该分类的总金额
                const categoryTotal = await convertExpensesToCurrency(categoryExpenses, selectedCurrency);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-gray-800 dark:text-white">支出统计</h2>
      </div>
          ...
          {stats.length > 0 ? (
            <div className="space-y-4">
                ...
                <div className="h-8 w-full flex rounded-full overflow-hidden">
                  {stats.map((item, index) => (
                    <div
                      ...
                      title={`${item.label}: ${formatCurrency(item.amount, selectedCurrency)} (${item.percentage.toFixed(1)}%)`}
                    ></div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {stats.map((item, index) => (
                  <div key={index} className="flex items-center p-2 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700">
                    <div className={`w-4 h-4 rounded-full ${getColorClass(index)} mr-2`}></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300 truncate">
                          {groupBy === 'category' ? item.label : formatDateLabel(item.label)}
                        </span>
